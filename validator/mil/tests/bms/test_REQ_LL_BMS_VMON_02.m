function r = test_REQ_LL_BMS_VMON_02()
% REQ-LL-BMS-VMON-02  UV warn / derate / shut after their debounce windows.
r = val.new_result("bms","BMS-VMON-02","UV monitor debounce ladder","REQ-LL-BMS-VMON-02");
...
V12(t >= t_warn_on   & t < t_warn_off,   3) = bms.V_UV_warn   - 0.01;
V12(t >= t_derate_on & t < t_derate_off, 3) = bms.V_UV_derate - 0.01;
V12(t >= t_shut_on,                      3) = bms.V_UV_shut   - 0.01;
T4  = 25*ones(N, 4);
en  = zeros(N, 1);
...
stim = struct('t', t, 'y', V12(:,3), 'label', 'V_{cell,3} [V]', ...
    'thresholds', {{bms.V_UV_warn,'UV warn'; bms.V_UV_derate,'UV derate'; bms.V_UV_shut,'UV shut'}});
...
r.signals_plot = string(val.scenario_plot(r, struct( ...
    'plot_dir', val.plot_root('mil','bms'), 'filename', 'BMS-VMON-02.png', ...
    'subtitle', 'cell-3 UV: warn / derate / shut after debounce', ...
