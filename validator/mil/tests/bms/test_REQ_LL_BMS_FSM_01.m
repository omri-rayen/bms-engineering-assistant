...
% Render the long (stage-2) timeline as the main scenario story:
% it covers warn -> derate -> shut -> latch on a single slave.
stim = struct('t', t2, 'y', double(ff2(:,1)), 'label', 'fault\_flags\_8(slave1)', ...
    'thresholds', {{1,'warn'; 2,'derate'; 3,'shut'}});
resp = struct('t', t2, 'y', sev2, 'label', 'fault\_severity', ...
    'thresholds', {{1,'warn'; 2,'derate'; 3,'shut'}});
phases = {1,         4,         'warn'; ...
          4,         t_shut_on, 'derate'; ...
          t_shut_on, t_clear,   'shut'; ...
          t_clear,   T2,        'cleared (latch test)'};
asserts = {4 + bms.N_debounce_derate*dt + dt,           ok_der,   'sev>=2'; ...
           t_shut_on + bms.N_debounce_shut*dt + dt,     ok_shut,  'sev>=3'; ...
           T2,                                          ok_latch, 'shut latched'};
r.signals_plot = string(val.scenario_plot(r, struct( ...
    'plot_dir', val.plot_root('mil','bms'), 'filename', 'BMS-FSM-01.png', ...
    'subtitle', 'severity FSM ladder: warn -> derate -> shut, latch survives clear', ...
