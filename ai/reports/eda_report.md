...
| Columns | 24 | 25 |
| Trips / Runs | 70 (32 TripA + 38 TripB) | 24 (3 temps x 4 scenarios x 2 bus) |
| NaN remaining | 0 | 0 |
...
### 3.1a TripA vs TripB Differences
...
### 3.1b Ageing Effects Between Campaigns
- In the overlapping temperature zone (17-22 C), TripB shows **+25 to +80 mV higher cell voltage** at the same SoC compared to TripA (median offset ~+30 mV per cell).
- This offset is attributed to **battery ageing or SoC-estimator drift** over the ~5-month gap.
- The effect is SoC-dependent: larger at high SoC (60-75 %), smaller at mid-SoC.
- Consequence: TripA and TripB data should **not** be assumed to come from the same battery state. OCV and ECM parameter extraction must account for this separation.
...
- TripB trips generally show higher current noise (std=3-5 A/step) vs TripA (std=2-3 A/step) due to cold-weather conditions.
